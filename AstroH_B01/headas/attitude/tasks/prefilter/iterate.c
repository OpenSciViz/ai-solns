...
int iterate_timestamp_delta (const Arguments * args, Context * context)
{
	...
	if (!context->timestamp.yr)
			missionTime = AtTime_to_missionTime(&args->basetime);
	else
		{
			missionTime = AtTime_to_missionTime(&context->timestamp);
			missionTime += args->interval;
		}

	missionTime_to_AtTime(missionTime, &context->timestamp);

	context->missionTime = missionTime;
	atCTime2(&context->timestamp, context->timestampString);
	atMJulian(&context->timestamp, &context->mjd);

	if (!context->code)
		if (AtTime_compare(&context->timestamp, &args->endtime) > 0)
...
{
	report_verbose("time %f [%s] [code %d]\n",
			context->missionTime, context->timestampString, context->code);
...
int iterate_position_tle (const Arguments * args, Context * context)
{
	...
	const AtTime * t = &context->timestamp;

	double sod = 3600 * t->hr + 60 * t->mn + t->sc + t->ms / 1000;
	datetime_set_year_month_day_sod(&timestamp, t->yr, t->mo, t->dy, sod);
...
int iterate_attitude_attfile (const Arguments * args, Context * context)
{
	if (!context->code)
		{
			if (!isInExtrapolatedAttFile(args->attfile, context->missionTime))
				{
	...
	if (!context->code)
		{
			if (findQuatInAttFile(args->attfile,
							context->coordquat, context->missionTime))
				{
