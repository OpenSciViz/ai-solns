...
C Calculate shell field L-value and magnetic field strength
      call geo_bl(dyear,dlong,dlat,dalt,dphi,   dl,db)
      FL=real(DL)
      B0=real(DB)
...
C new one to replace car_bl:
      SUBROUTINE geo_bl(time,dlong,dlat,dalt,dphi,   fl,b0)

C     IMPLICIT DOUBLE PRECISION (A-H,O-Z)

      IMPLICIT NONE
C     INPUT
      DOUBLE PRECISION time, dlong, dlat, dalt, dphi
C     OUTPUT
...
C     LOCAL
      ...
      common /bmodel/ modold, mmoold, tmold, bltime, ipass  
