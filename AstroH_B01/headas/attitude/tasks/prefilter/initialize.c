	...
	if (!filter_code(filter))
		{
			...
			p->derivedOffset = PREFILTER_OFFSET(missionTime);
...
static int initialize_atElement (Arguments * args)
{
	...
	if (atMJulian(&args->basetime, &mjd))
		{
			args->code = PREFILTER_SETUP_ERROR;
			report_error("unable to convert basetime to mjd\n");
		}
	else if ((*atFunction)(filename, mjd, 1))
		{
			args->code = PREFILTER_SETUP_ERROR;
...
static int initialize_tle_text (Arguments * args)
{
	...
	double latest = AtTime_to_yyyyddd_fraction(&args->basetime);
...
int initialize_time (Arguments * args)
{
	...
	if (!args->code)
		{
			...
			if (readLeapTable(args->leapname))
				{
					args->code = PREFILTER_SETUP_ERROR;
	...
	/* convert raw input times to basetime, endtime */
	if (!args->code)
		{
			missionTime_to_AtTime(args->start, &args->basetime);
			missionTime_to_AtTime(args->end, &args->endtime);
		}
