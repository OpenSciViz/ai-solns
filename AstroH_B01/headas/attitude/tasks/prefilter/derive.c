...
{
	derived->missionTime = context->missionTime;

	if (context->args->verbose)
		report_verbose("mission time %.3f\n", derived->missionTime);
	...
	if (atBrazil(polar->lon, polar->lat, &xflag))
	...
	else
		{
			if ((derived->lastSAA != constants->nulli)
				...
				derived->timeChangeSAA = context->missionTime;

			derived->inSAA = xflag;

			if (derived->timeChangeSAA != constants->nulle)
				derived->timeSinceSAA = context->missionTime - derived->timeChangeSAA;
		}
