...
int read_hdu_data(fitsfile * infptr, hduInfo *info, int * status)
{
    ...
    if ( info->hdutype == ASCII_TBL || info->hdutype == BINARY_TBL )
    {
          ...
          {
              sprintf(str,"%d",j+1);
              if(fits_get_colname(infptr,
                  1,str,info->column[j].name,&anyf,status)) return(*status);
              if(fits_get_coltype(infptr,j+1,&(info->column[j].dtype),
                  &anyl,&anyl,status)) return(*status);
              sprintf(str,"%s%d","TUNIT",j+1);
              if(fits_read_key(infptr,TSTRING,str,info->column[j].unit,
                  NULL,status)) 
              {  
    ...
    else if (info->hdutype == IMAGE_HDU)
    {
          ...
          if(info->naxis > 2 || info->naxis < 1) 
          {
              sprintf(msg,"NAXIS = %d, ftstat can be only applied to 1-D or 2-D image", 
              info->naxis);
              info->column = 0;
              *status =108;
              HD_ERROR_THROW(msg,*status);
...
int print_stat(FILE* outfptr, char* infile, hduInfo info, int centroid, int clip) {
 ...
 for (i=0; i<info.ncols; i++) {
    ...
    else  /* statistics of a table column */
    {
       ...
       if (info.column[i].dtype  == TSTRING || info.column[i].dtype  == TLOGICAL ) {
         continue;
