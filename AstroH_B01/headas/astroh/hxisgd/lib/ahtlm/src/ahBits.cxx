#if !defined(AHBITS_C)
#define AHBITS_C const char AHBITS_rcsId_svnId[] = "$Id: ahBits.cxx,v 1.5 2012/07/15 20:57:17 dhon Exp $"; 
...
namespace {
  const string cvsId = "$Name: AstroH_B01 $ $Id: ahBits.cxx,v 1.5 2012/07/15 20:57:17 dhon Exp $";
}
...
int bitText(vector<string>& printbuff, const BitsAny& bv, int stride, string prefix) {
  ...
  if( stride > slen ) {
    clog << __PRETTY_FUNCTION__ << " stride: " << stride << " exceeds text length" << slen << endl;
