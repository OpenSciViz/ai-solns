        ...
        while(!$error and !$eof) {
            ...
            while (!$error and length $buffer) {
                ...
                for my $target (@target) {
                    ...
                    $cp = $target->{prefix} . $cp if $target->{prefix} && length $target->{prefix};
...
AH::UTTee - Replicate data sent to a Perl stream.
            This is repackaged from CPAN's File::Tee version 0.07.
...
=head1 SYNOPSIS
  ...
  if( $NoOp || $Verbose ) { logStdOut(hello world\n";
  system "ls";
