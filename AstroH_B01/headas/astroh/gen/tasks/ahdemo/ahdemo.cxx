...
\defgroup tool_ahdemo Application Demo (ahdemo)
@ingroup mod_gen_tasks

Function of this application:
- Read rows from FFF file. Each row contains one "occurrence", which is some arbitrary number of "hits".
- Unpack each row (including variable-size array) into its "hits".
- Calibrate the hits to produce some number of single-photon events.
- Write each single-photon event to one row of the output (SFF) file, which thus has one photon event per row.

Assumptions for purposes of this mock-up:
- The FFF file contains two columns:
...
- The maximum possible number of hits in one row of the FFF is known to be MAXHIT. Each hit requires HITSIZE
   double values to encode its information. These hits will be converted to at most MAXEVENT events.
- The SFF file contains one fixed-length vector column named "EVENT". There is exactly one photon event per row. Each
   photon event requires EVENTSIZE double values to encode all its information.
- The output FITS file is created from a template file, by default input/sff-template.tpl.
   (Obviously this would need to look in some other standard place for this in a real tool.)
- There is a library libahgen that contains general purpose support functions.
- There is a library libahfits that works as follows:
...
- There is a library libahlog that has logging functions.
- Certain checks (for null pointers and other validations) have been omitted for now to improve readability.

C++ exception assumptions:
- There is an exception class AhException defined in libahgen that has desired error-handling properties.
...
/* Revision Log
   $Log: ahdemo.cxx,v $
   Revision 1.6  2012/07/20 14:54:39  mwitthoe
   add Doxygen to ahdemo
