...
Normally it stops and return's if a copy fails, to continue on regardless set $AH::UTRCopy::SkipFlop to true.

    local $AH::UTRCopy::SkipFlop = 1;
...
It forces behavior as if $AH::UTRCopy::CPRFComp is true.
...
Returns 2 if the given path is not a directory.
  ...
  AH::UTRCopy::pathrm('foo/bar/baz') or die $!;
...
An optional second argument makes it call pathempty() before any rmdir()'s when set to true.
  ...
  AH::UTRCopy::pathrm('foo/bar/baz', 1) or die $!;
  # foo no longer exists

Same as:PFSCheck

  AH::UTRCopy::pathempty('foo/bar/baz') or die $!;
  rmdir 'foo/bar/baz' or die $!;
  AH::UTRCopy::pathempty('foo/bar/') or die $!;
  rmdir 'foo/bar' or die $!;
  AH::UTRCopy::pathempty('foo/') or die $!;
  rmdir 'foo' or die $!;

An optional third argument acts like $AH::UTRCopy::NoFtlPth, again probably not a good idea.
...
Recursively removes the given directory's contents so it is empty. returns 2 if argument is not a directory, 1 on successfully emptying the directory.
   ...
   AH::UTRCopy::pathempty($pth) or die $!;
...
Creates a given path recursively. Creates foo/bar/baz even if foo does not exist.
   ...
   AH::UTRCopy::pathmk('foo/bar/baz') or die $!;

An optional second argument if true acts just like $AH::UTRCopy::NoFtlPth, which means you'd never get your die() if something went wrong. Again, probably a *bad* idea.
...
You can customize this behavior by setting $AH::UTRCopy::CopyLink to a true or false value.
It is already set to true or false dending on your system's support of symlinks so you can check it with an if statement to see how it will behave:
    ...
    if($AH::UTRCopy::CopyLink) {
        if( $NoOp || $Verbose ) { logStdOut(Symlinks will be preserved\n";
    } else {
        if( $NoOp || $Verbose ) { logStdOut(Symlinks will not be preserved because your system does not support it\n";
    }

If symlinks are being copied you can set $AH::UTRCopy::BdTrgWrn to true to make it carp when it copies a link whose target does not exist. Its false by default.

    local $AH::UTRCopy::BdTrgWrn  = 1;

=head2 Removing existing target file or directory before copying.

This can be done by setting $AH::UTRCopy::RMTrgFil or $AH::UTRCopy::RMTrgDir for file or directory behavior respectively.
...
It's a very efficient check that croaks if they are and shouldn't be turned off but if you must for some weird reason just set $AH::UTRCopy::PFSCheck to a false value. ("PFS" stands for "Physical File System")
...
You can make dircopy() emulate cp -rf by setting $AH::UTRCopy::CPRFComp to true.
...
type behavior set $AH::UTRCopy::CopyLoop to true.
